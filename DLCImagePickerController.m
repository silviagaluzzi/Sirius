...
-(void) sharedInit {
	outputJPEGQuality = 1.0f;
    requestedImageSize = CGSizeMake(640.0, 640.0); //CGSizeZero;
}
...
-(void)viewDidLoad {
    ...
    self.photoBar.backgroundColor = [UIColor whiteColor];
    ...
    // per la campagna San Valentino non erano richiesti, but who knows in future?
...
-(IBAction)toggleFlash:(UIButton *)button{
    ...
    if ([stillCamera.inputCamera hasTorch]) {
        NSError *error;
        [stillCamera.inputCamera lockForConfiguration:&error];
        if (!error) {
...
-(void)captureImage {
    ...
    void (^completion)(UIImage *, NSError *) = ^(UIImage *img, NSError *error) {
        ...
        if (skipPictureConfirmation == YES) {
            [self completePickingWithCurrentImage];
        } else {
...
//        [finalFilter prepareForImageCapture];
    ...
    /*} else {
        ...
        UIImage *img = [cropFilter imageFromCurrentFramebuffer]; // imageFromCurrentlyProcessedOutput];
        completion(img, nil);
    }
     */
}
...
-(void) completePickingWithCurrentImage {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (outputImage) {
        ...
        if (skipPictureConfirmation == YES ) {
            ...
            [self.photoCaptureButton setTitle:@"Share" forState:UIControlStateNormal];
            [self.photoCaptureButton setImage:nil forState:UIControlStateNormal];
            [self.photoCaptureButton setEnabled:YES];
            if(![self.filtersToggleButton isSelected]){
                [self showFilters];
...
/*    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        if (UIInterfaceOrientationIsLandscape(toInterfaceOrientation)) {
            [self.imageView setInputRotation: kGPUImageRotateLeft atIndex:0];
        }else{
            [self.imageView setInputRotation: kGPUImageRotateRight atIndex:0];
        }
    }
 */
}
